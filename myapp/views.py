...
from .models import Customer, Product, Cart, OrderPlaced, Contact, Blogs
...
def shop(request, data=None):
	...
	if data==None :
			...
			shop = Product.objects.filter(category='BW')
	elif data == 'tshirts':
			shop = Product.objects.filter(category='TW').filter(discounted_price__lt=10000)
	elif data == 'runningkits':
			shop = Product.objects.filter(category='TW')
	elif data == 'shoes':
			shop = Product.objects.filter(category='S').filter(discounted_price__gt=10000)
	return render(request, 'app/shop.html', {'shop':shop, 'totalitem':totalitem})
